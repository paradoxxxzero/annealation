...
#[wasm_bindgen]
impl P2PRustSimdGravity {
  pub fn new(len: usize) -> P2PRustSimdGravity {
    init_hook();
  ...
  unsafe fn _simulate(
    ...
    for i in 0..self.len {
      ...
      for j in 0..i {
        ...
        let b = [
          self.positions[j * 3],
          self.positions[j * 3 + 1],
          self.positions[j * 3 + 2],
          0f32,
        ];
        let vec_a: v128 = transmute(a);
        let vec_b: v128 = transmute(b);
        let v = f32x4_sub(vec_b, vec_a);
        let w = f32x4_mul(v, v);
        let u: [f32; 4] = transmute(w);
        let distance2 = u[0] + u[1] + u[2];
      ...
      for &item in collided.iter() {
        ...
        for k in 0..3 {
          ...
          self.speeds[i * 3 + k] = mass_ratio
            * (self.speeds[i * 3 + k] * self.masses[i] + self.speeds[j * 3 + k] * self.masses[j]);
        }
        self.temperatures[i] = mass_ratio
          * (self.temperatures[i] * self.masses[i] + self.temperatures[j] * self.masses[j]);
  ...
  pub fn simulate(
    ...
    unsafe {
      return self._simulate(g, softening, collisions, threshold, escape_distance);
    }
  }
